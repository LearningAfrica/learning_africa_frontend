...
<template>
	<main class="max-w-7xl w-full">
		...
		<div v-if="!is_loading"
			...
			<div
				...
				<div class="flex flex-wrap gap-2 ">
					<span v-for="tag of data.tags"
						  class="border px-1 rounded-lg bg-slate-50">{{ tag
						}}</span>
				</div>
				<p>
			...
			<div
				...
				<div class="flex items-center border-b justify-between p-2">
					...
					<button class="flex text-white items-center gap-2 bg-primary py-1 px-4 rounded"
				...
				<div v-if="data.modules && data.modules.length > 0"
					...
					<div v-for="{ title, id: module_id } of data.modules"
		...
		<hui-transition-root :show="addModuleOpen" appear as="template">
			<hui-dialog as="div" class="relative z-[100]"
						@close="toggleAddModule(false)">
				...
				<div
					...
					<hui-transition-child as="template"
						...
						<hui-dialog-panel
							...
							<form v-if="!is_add_module_loading" action=""
							...
							<spin-loader v-if="is_add_module_loading"
										 :controller="controller!"
										 :is_loading="is_add_module_loading"
