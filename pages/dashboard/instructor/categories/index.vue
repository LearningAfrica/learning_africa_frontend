					...
					<tr v-for="cat of categories!.data" :key="cat.id">
						...
						<td>
							<div class="flex gap-4 justify-center">
								...
								@click="selectCategoryToUpdate(cat.id)"
								>Update</button>
								<button class="border-red-500 border py-1 px-4 rounded-sm"
								@click="selectCategoryToDelete(cat.id)"
								>Delete</button>
							</div>
						</td>
					</tr>
			...
			<div class="bg-white border p-6 rounded-lg shadow-lg  w-full min-h-[20vh] max-w-xl flex gap-4 flex-col">
				<hui-dialog-panel>
					...
					<form action="" class="w-full"
						@submit.prevent.stop="handleDeleteCategory">
						<!-- <div class="w-full">
						...
						<div class="flex justify-center mt-6 gap-4">
							...
							<button type="submit" :disabled="isDeleting" :class="{ 'cursor-not-allowed': isDeleting }"
								class="px-4 py-2 
