...
export const useSingleCourseModule = () => {
	const {$privateAxios} = useNuxtApp();
	const singleCourseModuleData = useState(
		"single-course-module-data",
		() => ({}) as Data
	);
	const fetch_error = useState<AxiosError | Error | null>(
		"single-course-module-fetch-error",
		() => null
	);
	const has_error = useState<boolean>(
		"single-course-module-loading",
		() => false
	);
	...
	const fetchData = async (
		{course_id, module_id}: Params,
		bypass_cache: boolean = false
	) => {
		const cacheKey = `data:course_id=${course_id}&module_id=${module_id}`;
		...
		try {
			if (!bypass_cache) {
				...
				if (cache.value[cacheKey]) {
					...
					singleCourseModuleData.value = cache.value[cacheKey];
			...
			singleCourseModuleData.value = data;
